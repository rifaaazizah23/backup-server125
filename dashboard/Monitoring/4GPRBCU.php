      ...
      <!-- MULAI DARI SINI -->
              <div class="col-md-12">
                <div class="x_panel">
                  ...
                  <div class="x_content">
					<p>High PRB - High CU	= <img src="towerHigh.png" style="width:20px ; height : 20px">|
					High PRB - Low CU	= <img src="towerMediumHigh.png" style="width:20px ; height : 20px">|
					High CU - Low PRB	= <img src="towerMedium.png" style="width:20px ; height : 20px">|
					Low PRB & Low CU		= <img src="towerLow.png" style="width:20px ; height : 20px">
...
<script type="text/javascript">
      ...
      function getUsers() {
        $.getJSON("get_4gprbcu.php", function (data) {
          for (var i = 0; i < data.length; i++) {
            var location = new L.LatLng(data[i].lat, data[i].lng);
            var name = data[i].name;
			var eNodebName = data[i].name2;
			if ( data[i].remark == "High" ){		
				var  greenIcon = new LeafIcon({iconUrl: 'towerHigh.png'});
			} else if( data[i].remark == "MediumHigh" ){
				var  greenIcon = new LeafIcon({iconUrl: 'towerMediumHigh.png'});
			}else if ( data[i].remark == "Medium" ){
				var  greenIcon = new LeafIcon({iconUrl: 'towerMedium.png'});
			} else if ( data[i].remark == "Low" ){
				var  greenIcon = new LeafIcon({iconUrl: 'towerLow.png'});
			} else {
				var  greenIcon = new LeafIcon({iconUrl: 'towerNull.png'});
			}
			L.icon = function (options) {
						return new L.Icon(options);
					};
            if (data[i].name.length > 1) {
              var title = "<div style='font-size: 12px; color: #000000;'>name : <a href='Detail4G.php?name="+ data[i].name +"' target='_blank'>"+ data[i].name + "</a><br>eNodebName : "+data[i].name2 + "<br>Bandwidth : "+data[i].Bandwidth + "<br>HourId : "+data[i].HourId + "<br>LDL : "+data[i].LDL + "<br></div>";
            }
            else {
